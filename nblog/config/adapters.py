...
class AdapterMixin(object):
    ...
    def check_email(self, email, **kwargs):
        ...
        #     and settings.USERS_REQUIRE_VERIFICATION
        #     and not email.is_verified
        # ):
        #     msg = f"User {email} is not verified."
        #     # send_email_confirmation(request, user)
        #     if self.is_api:
        #         raise APIException(msg)
        #     else:
        #         response = self.respond_email_verification_sent(self.request, email)
        #         raise ImmediateHttpResponse(response)

        # approval (display a message)
        if (
            kwargs.get("check_approval", True)
            and settings.USERS_REQUIRE_APPROVAL
            and not email.is_approved
        ):
            msg = f"User {email} has not been approved."
            if self.is_api:
                raise APIException(msg)
            else:
        ...
        if (
            ...
            and settings.USERS_REQUIRE_TERMS_ACCEPTANCE
            and not email.accepted_terms
        ):
            msg = f"User {email} has not yet accepted the terms & conditions."
            if self.is_api:
                raise APIException(msg)
            else:
                # (this is dealt w/ in "authenticate" below as a ValidationError, but just-in-case...)
        ...
        if kwargs.get("check_password", True):
            self.send_password_confirmation_email(email, email.email)
            if self.is_api:
...
class AccountAdapter(AdapterMixin, DefaultAccountAdapter):
    ...
    def authenticate(self, request: HttpRequest, **credentials):
        email = super().authenticate(request, **credentials)

        if email is not None:
            if settings.USERS_REQUIRE_APPROVAL and not user.is_approved:
                msg = f"User {email} has not been approved."
                raise forms.ValidationError(msg)
            elif (
                settings.USERS_REQUIRE_TERMS_ACCEPTANCE
                and not email.accepted_terms
            ):
                msg = f"User {email} has not yet accepted Terms & Conditions."
                raise forms.ValidationError(msg)
